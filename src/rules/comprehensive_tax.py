...
⚠️ 데이터 불확실성: doc38 원문 표에서 "종신연금 수령 시" 특례 세율(3.3%)은 55~70세 구간에만
명시돼 있고, 70세 이상 구간은 셀이 비어 있어 특례가 계속 적용되는지 원문만으로는 확정할 수 없다.
과대 일반화를 피하기 위해 이 구현은 70세 이상에서는 종신연금 여부와 무관하게 일반 세율을
적용한다 (70세 이상은 어차피 일반 세율도 4.4%/3.3%로 낮아 실무 영향은 제한적). 확정이 필요하면
소득세법 시행령 원문을 추가로 대조할 것.
"""
...
def get_pension_income_tax_rate(age: int, is_lifetime_annuity: bool = False) -> float:
    ...
    if age < 70:
        return 0.033 if is_lifetime_annuity else 0.055
    if age < 80:
        return 0.044  # 종신연금 특례가 이 구간에도 적용되는지 원문상 불확실 (위 모듈 docstring 참고)
    return 0.033
