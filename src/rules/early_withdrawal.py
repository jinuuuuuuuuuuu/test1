...
⚠️ 기간 단위: 원문은 "1개월", "3개월", "5년"으로만 표현하고 30일/90일 환산, 초일산입,
말일·휴일 처리 방식을 정의하지 않는다. 따라서 일반 Agent의 DB-grounded 답변은 exact date를
단정하지 않고 "기준일로부터 N개월/년 이내"까지만 안내해야 한다.

아래 calculate_deadline 계열 함수는 30일/90일 고정 환산을 피하기 위한 개발자 참고용
calendar_reference 계산기다. 기본 Agent Tool은 이 값을 바로 노출하지 않는다.
