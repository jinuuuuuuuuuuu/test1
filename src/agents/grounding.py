...
4. unsupported_claims:
   [초안 답변]의 사실성 주장 중 [근거] 어느 항목에서도 직접적으로
   뒷받침되지 않는 주장을 찾아 unsupported_claims에 적으세요.

   특히 다음을 검사하세요.
   - 근거에 없는 목적·이유·원인 설명
   - 특정 상품 또는 일부 사례를 전체 제도에 일반화한 설명
   - 근거에 없는 가능/불가능 판단
   - 근거에 없는 세제·투자제한·가입조건 설명
   - 근거에 없는 장점·단점·평가
   - 미래 수익률이나 시장 전망에 대한 단정

   중요:
   "상식적으로 그럴 것 같다"는 이유로 supported로 판정하면 안 됩니다.
   제공된 [근거] 안에서 확인할 수 있어야 합니다.

   예:
   근거: "IRP 상호간 이체는 연령 제한 없이 가능"
   초안: "IRP 상호간 이체는 투자상품 변경을 위해 주로 사용된다"
   → 두 번째 내용은 근거에 없으므로 unsupported_claims에 포함합니다.

5. missing_evidence:
   질문의 핵심 요구사항을 정확하게 답하기 위해 필요한 정보인데
   현재 [근거]에서 확인할 수 없는 내용이 있다면 적으세요.

   단, 질문과 직접 관계없는 부가정보까지 필요하다고 판단하지 마세요.
   필요한 근거가 모두 있다면 빈 리스트로 둡니다.
"""
