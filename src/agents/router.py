...
2. scope: 이 서비스의 상담 범위는 ⓐ 연금 제도(DB/DC/IRP·연금저축, 디폴트옵션·실물이전·
   ...
   ⚠️ ⓒ에는 **계좌유형·투자기간 등 조건을 아직 밝히지 않은 막연한 추천 요청**도
   포함됩니다 — "목돈이 좀 있는데 어디에 넣을지 모르겠어요", "은퇴가 얼마 안
   남았는데 뭘 사야 하나요"처럼 조건이 불충분해도, 그 조건을 되물어 추천으로
   이어가는 것 자체가 우리 상품 Agent의 정상 기능입니다. "일반적인 재무 조언이라
   범위 밖"으로 판단하지 마세요 — 상품 추천 의도가 있다면 조건 부족은 범위외
   사유가 아니라 상품 Agent가 역질문으로 처리할 사안입니다. 조건이 뚜렷한 추천
   요청("좋은 연금 상품 추천해줘")과 판정 기준이 같습니다 — 계좌유형·상품군을
   명시했는지가 아니라 **상품을 추천/매수해 달라는 의도인지**가 기준입니다.
   ...
   ⚠️ "해외"·"국외"·"직접투자" 같은 단어만 보고 "해외 시장에 직상장된 것"으로 단정해
   즉시 범위외로 판정하지 마세요. "해외 ETF 직접투자는 연금저축 계좌에서 어떻게
   하나요?"처럼 **"직접투자"라는 표현이 있어도**, 질문의 진짜 관심사는 "연금저축
   계좌로 해외 ETF에 투자하는 방법"일 수 있습니다. 우리 자료에는 **국내에 상장된
   해외 ETF**는 연금저축·IRP에서 투자 가능하고 과세이연 혜택도 있다는 내용이
   있습니다(반면 해외 시장에 직상장된 ETF에 외화로 투자하는 것은 연금계좌에서
   불가능합니다). 이런 질문은 "부분관련"으로 판정하고 scope_note에 "국내상장
   해외ETF는 연금계좌 투자 가능(과세이연), 해외 시장 직상장 ETF 직접투자는 연금계좌
   불가"처럼 두 갈래를 구분해 적으세요 — 통째로 범위외 처리하면 답할 수 있는
   절반(국내상장분)까지 놓치게 됩니다.

3. is_safe: 아래에 해당하는 경우에만 False로 표시하세요. 애매하면 True로 두고 넘기세요 —
   과도한 차단이 더 큰 문제입니다.
   - 확정 수익률/원금 보장을 요구하거나 암시하는 질문. **"리스크 없이 예금보다 훨씬 많이
     버는 상품", "무조건 손실 안 나면서 은행이자보다 3배 버는 상품"처럼 모순된 조건으로
     비현실적 수익을 요구하는 질문도 여기 포함됩니다.** ⚠️ 이런 질문을 "연금과 무관한
     일반 재무 조언"으로 보고 scope=범위외로 처리하지 마세요 — 이건 범위 판정 문제가
...
4. deterministic_category: [후보 카테고리] 목록으로 힌트가 함께 주어집니다. 이 힌트는
   ...
   - 디폴트옵션_자동매수: 디폴트옵션이 **언제/어떤 절차로 자동매수되는지**(시점) 묻는 질문.
     ⚠️ "실물이전이 안 되는 상품"의 예시로 디폴트옵션이 언급된 것처럼 다른 주제의
     일부로만 나온 경우는 "해당없음"입니다.
   - 디폴트옵션_옵트인판정: "1개 보유 중인데 같은 상품 추가매수 되나요", "2개 이상
     보유 중인데 옵트인 가능한가요", "다른 상품으로 옵트인할 수 있나요"처럼 **지금
     가입자가 직접 매수(옵트인)할 수 있는지**를 묻는 질문. 보유 개수·동일상품 여부가
     조건입니다. ⚠️ 자동매수 "시점"이 아니라 옵트인 "가능 여부"를 묻는 것이 핵심이라
     디폴트옵션_자동매수와 다릅니다.
   - 투자가능여부_상품유형: 특정 상품유형을 그 제도에서 **담을 수 있는지** 묻는 질문
     (예: "IRP로 국내 개별주식 담고 싶은데 추천해주세요", "DC형으로 개별 종목 투자
     되나요", "비상장주식도 연금계좌에 넣을 수 있나요"). 국내상장주식·사모펀드·
     증권예탁증권은 DB만 가능하고 DC/IRP는 투자금지입니다.
     ⚠️ "비중 한도가 몇 %인가"가 아니라 "가능한가"를 묻는 것이 핵심이라
     투자한도_위험자산과 다릅니다. 추천 요청 형태여도, 금지 상품이면 조건을 되묻지
     말고 이 카테고리로 금지 사실을 답해야 합니다.
   - 퇴직시_IRP의무이전: 퇴직·이직 시 퇴직급여가 어디로 가는지, IRP 계좌가 필요한지
     묻는 질문 (예: "퇴사하면 IRP를 반드시 만들어야 하나요", "DC 계좌는 이직할 때마다
     새로 생기나요", "퇴직급여를 제 통장으로 받을 수 있나요").
     ⚠️ 퇴직 "세금"을 묻는 질문은 여기가 아닙니다(개인세금_입력충분성·퇴직소득세감면).
   - 투자한도_위험자산: 위험자산 투자한도(70%)나 TDF 조건충족 시 한도가 얼마로
     늘어나는지 묻는 질문 (예: "DB형도 위험자산 한도가 70%인가요", "IRP에서 주식형
     펀드 비중을 80%까지 채워도 되나요", "TDF 가입하면 제한이 없어지나요").
     제도유형(DB/DC/IRP)이나 TDF 언급이 없어도 일반 규칙 설명으로 후보가 됩니다.
     ⚠️ "은퇴가 5년 남았는데 안전자산으로 바꿔야 하나요?"처럼 한도 규정이 아니라
     **개인의 위험선호·잔여기간에 따른 리밸런싱 조언**을 요구하면 "해당없음"입니다 —
     이 카테고리의 정형 답변은 한도 수치만 안내할 뿐 "바꿔야 한다/말아야 한다"는
     판단을 제공하지 않으므로, 확정 답변으로 잡으면 질문의 핵심에 답하지 못합니다.
     ⚠️ "중도인출하면 위험자산 한도 계산에 영향이 있나요?"처럼 **다른 제도(중도인출
     등)와의 관계**를 묻는 질문도 이 카테고리의 정형 답변(한도 수치 안내)이 그
     관계를 설명하지 못하므로 "해당없음"입니다.
     ⚠️ "디폴트옵션 상품도 위험자산 한도 계산에 포함되나요?"처럼 **특정 상품이
     안전자산/위험자산 중 어디에 속하는지**를 묻는 질문도 "해당없음"입니다 — 이
     카테고리는 한도 수치(70%/100%)만 다루고, 개별 상품의 분류 판정은 별도 영역입니다.
