...
> **더 많이 말하는 AI가 아니라, 틀릴 때 멈추고 놓칠 때 먼저 알려주는 AI입니다.**
...
| **파수꾼(Guardian)** | 본 답변이 검증된 뒤, 사용자가 묻지 않았더라도 놓치면 손해가 될 수 있는 혜택·세금·비용을 최대 1건 추가로 안내하는 후단 기능입니다. |
...
<p align="center">
  <img src="docs/images/model_architecture.png" alt="pension-agent 모델 구조도" width="720"/>
</p>
...
파수꾼은 사용자가 질문하지 않았지만 지금 놓치면 손해가 될 수 있는 지점을 검사합니다.
...
### 💰 Cost Guard 데이터셋 — 저비용 판매 클래스 점검용
...
`.env`에 CLOVA Studio API Key를 설정합니다. 두 키 모두 필수입니다 — `CLOVASTUDIO_API_KEY`는 채팅·검증 모델(HCX-007/HCX-005) 호출에, `CLOVASTUDIO_EMBEDDING_API_KEY`는 RAG 검색 시 임베딩 호출에 쓰입니다.

```env
CLOVASTUDIO_API_KEY=your_api_key
CLOVASTUDIO_EMBEDDING_API_KEY=your_embedding_api_key
```
...
파수꾼은 사용자의 질문을 대신 바꾸거나 다른 상품을 자유롭게 추천하지 않습니다. **질문에 대한 정확한 답을 먼저 완성한 뒤**, 검증된 규칙으로 놓친 위험이나 혜택만 덧붙입니다.

---

## ✨ 차별점

| 일반적인 연금 RAG 챗봇 | 🛡️ 연금 파수꾼 |
| --- | --- |
...
3. **질문 밖의 손실까지 보는 파수꾼** — 검증된 본 답변 이후에만, 사용자가 놓친 손실·혜택·비용을 최대 1건 선제 안내합니다.
